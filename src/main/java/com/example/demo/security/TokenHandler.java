...
public final class TokenHandler {
    ...
    public MyUserDetails parseUserFromToken(String token) {
        ...
        if (parts.length == 2 && parts[0].length() > 0 && parts[1].length() > 0) {
            try {
                ...
                if (validHash) {
                    final MyUserDetails user = fromJSON(userBytes);
    ...
    public String createTokenForUser(MyUserDetails user) {
    ...
    private MyUserDetails fromJSON(final byte[] userBytes) {
        try {
            return new ObjectMapper().readValue(new ByteArrayInputStream(userBytes), MyUserDetails.class);
        } catch (IOException e) {
            throw new IllegalStateException(e);
        }
    }

    private byte[] toJSON(MyUserDetails user) {
        try {
